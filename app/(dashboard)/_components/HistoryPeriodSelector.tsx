    ...
    <div className="flex flex-wrap items-center gap-2">
      ...
      </SkeletonWrapper>
      {
        timeframe === "month" && (<SkeletonWrapper isLoading={historyPeriods.isFetching} fullWidth={false}>
          <MonthSelector period={period} sePeriod={setPeriod}/>
        </SkeletonWrapper>)
      }
    </div>
  </div>);
...
function YearSelector({period, setPeriod, years} : {
  ...
  return (<Select value={period.year.toString()} onValueChange={value => {
...
function MonthSelector({period, setPeriod} : {
  ...
  return (<Select value={period.month.toString()} onValueChange={value => {
      return setPeriod({year: period.year, month: parseInt(value)});
    }}>
    <SelectTrigger className="w-[180px]">
      <SelectValue/>
    </SelectTrigger>
